...
func NewTraceDispatcher(traceCfg *primitive.TraceConfig) *traceDispatcher {
	...
	if len(traceCfg.NamesrvAddrs) > 0 {
		srvs, err = NewNamesrv(primitive.NewPassthroughResolver(traceCfg.NamesrvAddrs), traceCfg.IgnoreCheckAddr)
	} else {
		srvs, err = NewNamesrv(traceCfg.Resolver, traceCfg.IgnoreCheckAddr)
	}
